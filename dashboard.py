...
def display_ui():
    ...
    with chart_col1:
        ...
        buckets:dict = {}
        with st.session_state.lock:  #acquiring the lock to read the shared state variables in a thread safe way
            tpm_snapshot = list(st.session_state.tpm_history)  #getting the transactions per minute history stored in the session state to show it in the dashboard as a line chart
        if tpm_snapshot:
            for event in tpm_snapshot:  #as we have stored the transactions per minute history in the session state as a list of dict with time and count keys, we are iterating over this list to prepare the data for line chart to show the total count of transactions received in each minute
    ...
    with chart_col2:
        ...
        with st.session_state.lock:
            type_counts_snapshot = dict(st.session_state.type_counts)  #getting the transaction type counts for fraud and legit transactions stored in the session state to show it in the dashboard as a bar chart
        if type_counts_snapshot:
            datas = [{'type':t, 'fraud':v['fraud'], 'legit':v['legit']} for t,v in type_counts_snapshot.items()]  #preparing the data for the bar chart to show the count of fraud and legit transactions for each type of transaction
    ...
    with st.session_state.lock:
        messages_snapshot = list(st.session_state.messages)  #getting the recent transactions feed stored in the session state to show it in the dashboard as a table
    if messages_snapshot:
            messages = list(messages_snapshot)[:50] #getting the latest 50 messages stored in the session state to show it in the dashboard as a feed as we have stored the messages from left side
            rows = []
            for msg in messages:
